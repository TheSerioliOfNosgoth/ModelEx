...
namespace BenLincoln.TheLostWorlds.CDTextures
{
    public class Gex3PlaystationVRMTextureFile2 : BenLincoln.TheLostWorlds.CDTextures.TextureFile
    {
        ...
        public void BuildTexturesFromPolygonData(Gex3PlaystationPolygonTextureData2[] texData, ushort[] texturePages, bool drawGreyScaleFirst, bool quantizeBounds, CDC.Objects.ExportOptions options)
        {
            if (_TPages == null)
            {
                List<TexturePage> tPages = new List<TexturePage>();
            ...
            foreach (Gex3PlaystationPolygonTextureData2 poly in texData)
            {
        ...
        protected Bitmap GetTextureAsBitmap(int index, Color[] palette)
        {
            ...
            for (int y = 0; y < _ImageHeight; y++)
            {
                ...
                {
                    int pixel = texturePage.pixels[y, x];
                    retBitmap.SetPixel(x, y, palette[pixel]);
                }
            }
